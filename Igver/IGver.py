...
from IGgetuser import win_main
# headers = {
...
def create_shutdown_and_extract_script(zip_file_path):
    script_content = (
        f'@echo off\n'
        f'echo 关闭当前程序...\n'
        f'taskkill /F /IM IGver.exe\n'  # 确保进程名称正确
        f':CHECK\n'
        f'tasklist | findstr /I /C:"IGver.exe" >nul\n'
        f'if %errorlevel%==0 (\n'
...
def create_registry_file():
    ...
    sstap_path = get_absolute_path("IGver.exe")
    sstap_path_registry = sstap_path.replace("\\", "\\\\")

    script_content = (
        'Windows Registry Editor Version 5.00\n\n'
        '[HKEY_CLASSES_ROOT\\IGProtocol]\n'
        '@="URL:IGProtocol Protocol"\n'
        '"URL Protocol"=""\n\n'
        '[HKEY_CLASSES_ROOT\\IGProtocol\\DefaultIcon]\n'
        f'@="{sstap_path_registry},1"\n\n'
        '[HKEY_CLASSES_ROOT\\IGProtocol\\shell]\n\n'
        '[HKEY_CLASSES_ROOT\\IGProtocol\\shell\\open]\n\n'
        '[HKEY_CLASSES_ROOT\\IGProtocol\\shell\\open\\command]\n'
        f'@="\\"{sstap_path_registry}\\" \\"%1\\""\n'
    )
...
def install_new_version_thread(version, remote_version):
    def install():
        ...
        if answer:
            ...
            def start_download():
                url = f"http://ver.kydb.vip/UpData/FBfans/IGver.zip?x={int(time.time())}"
                local_path = 'IGver.zip'
