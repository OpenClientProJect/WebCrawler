...
class MyApp(QWidget):
    def __init__(self):
        super().__init__()
        self.setWindowIcon(QIcon(".assets/IGicon.ico"))
        self.initUI()
        self.center()
    ...
    def initUI(self):
        ...
        self.browser_dropdown.addItem("隱藏輸入的链接地址框")
        self.browser_dropdown.addItem("顯示輸入的链接地址框")
        self.browser_dropdown.currentIndexChanged.connect(self.toggle_browser_input)
        main_layout.addWidget(QLabel("指定作品的點贊用戶列表和評論用戶列表:"))
        main_layout.addWidget(self.browser_dropdown)

        # 隐藏的浏览器数输入框
        self.hidden_browser_input = QTextEdit(self)
        self.hidden_browser_input.setPlaceholderText("請輸入點贊用戶評論地址...")
    ...
    async def on_click(self):
        ...
        self.hide()  # 隐藏窗口而非关闭
        try:
            await main(content0, content1, lines_hidden_line, lines_hidden_text)
