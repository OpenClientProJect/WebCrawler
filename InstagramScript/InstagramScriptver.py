...
def create_shutdown_and_extract_script(zip_file_path):
    script_content = (
        f'@echo off\n'
        f'echo 关闭当前程序...\n'
        f'taskkill /F /IM InstagramScriptver.exe\n'  # 确保进程名称正确
        f':CHECK\n'
        f'tasklist | findstr /I /C:"InstagramScriptver.exe" >nul\n'
        f'if %errorlevel%==0 (\n'
        f'    timeout /T 1 /NOBREAK >nul\n'
        f'    goto CHECK\n'
        f')\n'
        f'echo 删除旧程序...\n'
        f'del "%~dp0InstagramScriptver.exe"\n'  # 使用 %~dp0 获取批处理文件所在目录
        f'echo 解压新程序...\n'
        f'powershell -Command "Expand-Archive -Path \'%~dp0{zip_file_path}\' -DestinationPath \'%~dp0\' -Force"\n'
        f'echo 启动新程序...\n'
        f'start \"\" \"%~dp0InstagramScriptver.exe\"\n'  # 使用绝对路径启动
        f'echo 删除压缩包...\n'
...
def create_registry_file():
    ...
    sstap_path = get_absolute_path("InstagramScriptver.exe")
    sstap_path_registry = sstap_path.replace("\\", "\\\\")

    script_content = (
        'Windows Registry Editor Version 5.00\n\n'
        '[HKEY_CLASSES_ROOT\\InstagramScriptProtocol]\n'
        '@="URL:InstagramScriptProtocol Protocol"\n'
        '"URL Protocol"=""\n\n'
        '[HKEY_CLASSES_ROOT\\InstagramScriptProtocol\\DefaultIcon]\n'
        f'@="{sstap_path_registry},1"\n\n'
        '[HKEY_CLASSES_ROOT\\InstagramScriptProtocol\\shell]\n\n'
        '[HKEY_CLASSES_ROOT\\InstagramScriptProtocol\\shell\\open]\n\n'
        '[HKEY_CLASSES_ROOT\\InstagramScriptProtocol\\shell\\open\\command]\n'
        f'@="\\"{sstap_path_registry}\\" \\"%1\\""\n'
    )
...
def install_new_version_thread(version, remote_version):
    def install():
        ...
        if answer:
            ...
            def start_download():
                url = f"http://ver.kydb.vip/UpData/FBfans/InstagramScriptver.zip?x={int(time.time())}"
                local_path = 'InstagramScriptver.zip'
...
async def check_version():
    version = '1.0.0.7'
    remote_version_url = 'http://ver.ry188.vip/API/getver.aspx?N=InstagramScript'
