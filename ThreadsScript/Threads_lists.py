...
class DropArea(QFrame):
    ...
    def __init__(self, parent=None):
        ...
        self.setStyleSheet("""
            DropArea {
                background-color: #fff9f0;  /* 改为白色背景 */
                margin: 5px;
        ...
        self.container.setStyleSheet("""
            background-color: #fff9f0;  /* 改为白色背景 */
            border: 2px dashed #ffcc80;  /* 在内层容器添加虚线边框 */
            border-radius: 10px;
        """)
...
class TaskOrderWindow(QDialog):
    ...
    def initUI(self):
        # 设置窗口背景颜色
        self.setStyleSheet("background-color: #fef4e5; color: white;")
        ...
        # 右侧可拖拽标签区域
        right_panel = QFrame()
        right_panel.setStyleSheet("background-color: #fff5e6;")
        right_layout = QVBoxLayout(right_panel)
        right_layout.setContentsMargins(10, 10, 10, 10)

        label = QLabel("拖拽任務到左側區域:")
        label.setStyleSheet("color: black;font-size: 13px;")  # 改为蓝色字体
        ...
        if self.config:
            ...
            show_labels[1] = self.config.get("IsKey", False)
            show_labels[2] = self.config.get("IsPersonalSend", False)
            # 索引3（休息时间）始终为True，不需要设置
