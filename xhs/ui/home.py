    ...
    def update_config_file(self, crawl_type: str, value: str) -> bool:
        ...
        try:
            if crawl_type == "detail":
                ...
                # 更新 base_config.py 中的關鍵詞
                with open(self.base_config_path, 'r', encoding='utf-8') as f:
                    content = f.read()
                # KEYWORDS = "java" 格式，支持注释
                pattern = r'(KEYWORDS\s*=\s*")(.*?)(".*?)(\n)'
                new_content = re.sub(pattern, r'\1' + value + r'\3\4', content, flags=re.MULTILINE)
                # 如果上面的匹配失败（可能没有注释），尝试简单匹配
                if new_content == content:
                    pattern = r'(KEYWORDS\s*=\s*")(.*?)(")'
                    new_content = re.sub(pattern, r'\1' + value + r'\3', content)
